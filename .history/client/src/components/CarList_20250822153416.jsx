...
import { FaEdit, FaTrash, FaCarSide, FaTimes } from "react-icons/fa";
...
const CarList = () => {
  ...
  const handleDelete = async (carId) => {
    if (window.confirm("Are you sure you want to delete this car?")) {
      try {
        ...
        toast.success("Car deleted successfully!"); // Changed from alert()
        getCars();
      } catch (err) {
        toast.error("Failed to delete the car."); // Changed from alert()
      }
    }
  };
  ...
  const handleUpdate = async (updatedData) => {
    try {
      ...
      toast.success("Car updated successfully!"); // Changed from alert()
      setEditingCar(null);
      getCars();
    } catch (err) {
      toast.error("Failed to update car."); // Changed from alert()
    }
  };

  // Inline component for editing the car
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {Object.keys(formData).map((key) => (
            <div key={key}>
              ...
              <input
                type={
                  key === "price_per_day" || key === "year" ? "number" : "text"
                }
                id={key}
                name={key}
                value={formData[key]}
  ...
  // Conditionally render the edit form or the car list
  return (
    <div>
      {editingCar ? (
        <CarEditForm

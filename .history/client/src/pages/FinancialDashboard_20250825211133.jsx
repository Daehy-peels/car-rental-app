  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white p-4 border border-gray-300 rounded-lg shadow-xl">
        ...
        <p className="text-teal-500 mt-1">{`Revenue: $${value.toFixed(2)}`}</p>
      </div>
    );
  }
  return null;
...
const FinancialDashboard = () => {
  ...
  // A helper function to check if data exists before rendering a chart
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-4 sm:p-6 rounded-xl shadow-lg border border-gray-200 animate-fade-in">
          ...
          {hasData(data?.carPopularity) ? (
            ...
            <div className="flex justify-center items-center h-[300px]">
              <p className="text-gray-500">No car popularity data available.</p>
            </div>
          )}
        </div>

        {/* Revenue by Car Type Pie Chart */}
        <div className="bg-white p-4 sm:p-6 rounded-xl shadow-lg border border-gray-200 animate-fade-in">
          <h4 className="text-lg sm:text-xl font-bold text-gray-800 mb-6">
            Revenue by Car Type
          </h4>
          {hasData(data?.revenueByCarType) ? (
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
                  data={data.revenueByCarType}
            ...
            <div className="flex justify-center items-center h-[300px]">
              <p className="text-gray-500">
                No revenue by car type data available.
              </p>
            </div>
          )}
        </div>

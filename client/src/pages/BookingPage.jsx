...
import { fetchCars, createBooking, fetchCarBookings } from "../services/api";
...
const BookingPage = ({ currentUser }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        // Fetch all cars to find the selected one
        ...
        // Fetch existing bookings for this specific car
        const bookings = await fetchCarBookings(carId);
        setExistingBookings(bookings);
      } catch (err) {
        console.error(err);
        toast.error("Failed to fetch car details or bookings.");
        navigate("/");
      } finally {
        setLoading(false);
      }
  ...
  const isDateRangeBooked = (start, end) => {
    ...
    for (const booking of existingBookings) {
      ...
      // Check for overlap
      if (
        (newStart <= bookedEnd && newEnd >= bookedStart)
      ) {
        return true;
      }
    }
  ...
  const handleBookingSubmit = async (e) => {
    ...
    if (isDateRangeBooked(bookingDetails.startDate, bookingDetails.endDate)) {
        toast.error("The selected dates are already booked. Please choose a different range.");
        return;
    }
        ...
        <div className="space-y-6">
          <form onSubmit={handleBookingSubmit} className="space-y-4">
            ...
            {existingBookings.length > 0 && (
              <div className="bg-yellow-50 p-4 rounded-md text-sm text-yellow-700">
